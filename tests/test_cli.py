...
def test_calibrate_pen_writes_motion_profile(monkeypatch, tmp_path, capsys) -> None:
    ...
    exit_code = cli.main(["calibrate-pen", "--output", str(output_path), "--midpoint", "5.0", "--step", "0.5"])
    ...
    assert "Saved motion profile" in capsys.readouterr().out


def test_calibrate_pen_uses_safe_limits_when_user_keeps_advancing(monkeypatch, capsys) -> None:
    controller = FakeCalibrationController()
    answers = iter(["", "", "", "", "", "", "", "", "", "", "", "", ""])
    ...
    exit_code = cli.main(["calibrate-pen", "--midpoint", "5.0", "--step", "1.0"])
...
def test_mark_bounds_requires_known_plotting_origin(monkeypatch) -> None:
    controller = FakeBoundsController()

    monkeypatch.setattr(cli, "_controller_from_args", lambda args: controller)

    try:
        cli.main(["mark-bounds", "--model", "default", "--inset-mm", "5.0"])
    except ValueError as exc:
        message = str(exc)
    else:
        raise AssertionError("mark-bounds should reject raw-home execution")

    assert controller.calls == []
    assert "cannot start from raw home" in message
...
def test_run_program_dry_run_prints_normalized_commands(tmp_path, capsys) -> None:
    ...
    program_path.write_text("FORWARD 10 WIDTH 0.7\nRIGHT 90\n", encoding="utf-8")
    ...
    output = json.loads(capsys.readouterr().out)
    assert output == [
        {"line": 1, "command": "FORWARD", "values": [10.0, 0.7], "children": []},
        {"line": 2, "command": "RIGHT", "values": [90.0], "children": []},
    ]
...
def test_mark_bounds_uses_saved_workspace_profile_by_default(monkeypatch, tmp_path) -> None:
    ...
    (config_dir / "workspace.json").write_text(
        json.dumps({"model": "default", "width_mm": 120.0, "height_mm": 90.0}),
        encoding="utf-8",
    )

    monkeypatch.setattr(cli, "_controller_from_args", lambda args: controller)

    exit_code = cli.main(["mark-bounds", "--skip-home", "--config-dir", str(config_dir), "--inset-mm", "10"])
...
def test_mark_bounds_uses_inferred_model_when_requested(monkeypatch, capsys) -> None:
    controller = FakeBoundsController(inferred_model="v3a3")
    ...
    exit_code = cli.main(["mark-bounds", "--skip-home", "--use-model-preset", "--inset-mm", "10"])
...
def test_mark_bounds_uses_remembered_model_before_inference(monkeypatch, tmp_path) -> None:
    controller = FakeBoundsController(inferred_model=None)
    config_dir = tmp_path / ".drawcore"
    config_dir.mkdir()
    (config_dir / "devices.json").write_text(
        json.dumps({"USB VID:PID=1A86:8040 SER=01": "sea2"}),
        encoding="utf-8",
    )
    ...
    exit_code = cli.main(["mark-bounds", "--skip-home", "--use-model-preset", "--config-dir", str(config_dir)])

    bounds = workspace_bounds_for_model("sea2")
    assert exit_code == 0
    assert controller.calls == [
        ("pen_up", None, None),
        ("move_relative", 5.0, 5.0),
